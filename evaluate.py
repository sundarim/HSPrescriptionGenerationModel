...
def eval_AUPRC(y_true, y_score, method='LG', plot=True, **kargs):
    ...
    if plot:  # plot the precision-recall curves
        ...
        if prefix_id:
            prefix = f'{prefix_id}-precision_recall_curve-train' % prefix_id if phase.startswith(
                'tr') else f'{prefix_id}-precision_recall_curve'
        else:
...
def one_vs_all_encoding(df, target_label, codebook={'pos': 1, 'neg': 0}, col='ICD10', col_target='target'):
    ...
    if isinstance(df, DataFrame):
        ...
        print("> target: {} (dtype: {}) | n(pos): {}, n(neg): {}".format(target, type(target), np.sum(cond_pos),
                                                                         np.sum(cond_neg)))
...
def demo_evaluate_health_status_prediction(target_codes=[], model=None, show_plot=False, n_folds=5):
    ...
    if not target_codes:
        target_codes = ['00880ZZ']

    for target_code in target_codes:
        try:
            X, y = load_XY(suffix=target_code)
        except:
            print(f"[demo] Trouble loading data set for diagnosis code: {target_code}!")
            continue
